...
template<u32 BUCKETSIZE>
using bucketrow = bucket<BUCKETSIZE>[NY];
template <u32 BUCKETSIZE>
using matrix = bucketrow<BUCKETSIZE>[NX];
...
struct indexer {
  ...
  void matrixv(const u32 y) {
    const bucketrow<BUCKETSIZE> *foo = 0;
    for (u32 x = 0; x < NX; x++)
      index[x] = foo[x][y].bytes - (u8 *)foo;
  }
  offset_t storev(bucketrow<BUCKETSIZE> *buckets, const u32 y) {
  ...
  offset_t storeu(bucketrow<BUCKETSIZE> *buckets, const u32 x) {
    u8 const *base = (u8 *)buckets;
    offset_t sumsize = 0;
    for (u32 y = 0; y < NY; y++)
...
public:
  ...
  bucketrow<ZBUCKETSIZE> *bucketmatrix;
  bucketrow<TBUCKETSIZE> *tmpbuckets;
  zbucket32 *tedges;
  zbucket16 *tzs;
  zbucket8 *tdegs;
  offset_t *tcounts;
...
#if !defined(_WIN32)
    void* newobj;
    int tmp = posix_memalign(&newobj, NSIPHASH * sizeof(u32), sizeof(trimmer_ctx));
    if (tmp != 0) return nullptr;
#else
    void* newobj = _aligned_malloc(sizeof(trimmer_ctx), NSIPHASH * sizeof(u32));
  ...
  trimmer_ctx(const u32 n_threads, const u32 n_trims, const bool show_all) : barry(n_threads) {
    assert(sizeof(matrix<ZBUCKETSIZE>) == NX * sizeof(bucketrow<ZBUCKETSIZE>));
    assert(sizeof(matrix<TBUCKETSIZE>) == NX * sizeof(bucketrow<TBUCKETSIZE>));
    nthreads = n_threads;
    ntrims   = n_trims;
    showall = show_all;
    bucketmatrix  = new bucketrow<ZBUCKETSIZE>[NX];
    touch((u8 *)bucketmatrix, sizeof(matrix<ZBUCKETSIZE>));
    tmpbuckets = new bucketrow<TBUCKETSIZE>[nthreads];
    touch((u8 *)tmpbuckets, nthreads * sizeof(bucketrow<TBUCKETSIZE>));
#ifdef SAVEEDGES
    tedges  = 0;
#else
...
#endif
      sumsize += dst.storev(bucketmatrix, my);
    }
    rdtsc1 = __rdtsc();
...
// printf("id %d ux %d y %d edge %08x e' %010lx vx %d\n", id, ux, uy, *readedge, uy34 | ((u64)(node & YZMASK) << ZBITS) | *readz, vx);
      ...
      sumsize += dst.storeu(bucketmatrix, ux);
    }
    rdtsc1 = __rdtsc();
  ...
  void trimedges(const u32 id, const u32 round) {
    ...
    for (u32 vx = startvx; vx < endvx; vx++) {
      ...
      sumsize += TRIMONV ? dst.storev(bucketmatrix, vx) : dst.storeu(bucketmatrix, vx);
    }
            ...
            if (vdeg < 32) {
              ...
              *renames++ = vy << ZBITS | vz;
              if (renames == endrenames) {
                endrenames += (TRIMONV ? sizeof(bucketrow<ZBUCKETSIZE>) : sizeof(bucket<ZBUCKETSIZE>)) / sizeof(u32);
...
public:
  ...
  solver_ctx(char *header, int easipct = 50, const u32 n_threads = 4, const u32 n_trims = 68, bool allrounds = false, bool show_cycle = true) {
    trimctx = new trimmer_ctx(n_threads, n_trims, allrounds);
    showcycle = show_cycle;
    cuckoo = 0;
    setheader(header, HEADERLEN, &trimctx->sip_keys);
    sols.clear();
  }
  ...
  u32 threadbytes() const {
    return sizeof(thread_ctx) + sizeof(bucketrow<TBUCKETSIZE>) + sizeof(zbucket8) + sizeof(zbucket16) + sizeof(zbucket32);
  }
  void recordedge(const u32 i, const u32 u2, const u32 v2) {
    ...
    assert(vyz < NYZ1);
#if COMPRESSROUND > 0
    uyz = trimctx->bucketmatrix[ux][uyz >> Z1BITS].renameu[uyz & Z1MASK];
    vyz = trimctx->bucketmatrix[vyz >> Z1BITS][vx].renamev[vyz & Z1MASK];
#endif
...
// printf("vx %02x ux %02x e %08x uxyz %06x vxyz %06x u0 %x v0 %x nu %d nv %d\n", vx, ux, e, uxyz, vxyz, u0, v0, nu, nv);
  ...
  int run() {
    assert((u64)CUCKOO_SIZE * sizeof(u32) <= trimctx->nthreads * sizeof(bucketrow<TBUCKETSIZE>));
    trimctx->trim();
  ...
  void *matchUnodes(match_ctx *mc) {
    ...
    siphash_keys &sip_keys = trimctx->sip_keys;
    __m128i v0, v1, v2, v3, v4, v5, v6, v7;
    const u32 e2 = 2 * edge;
    __m128i vpacket0 = _mm_set_epi64x(e2+2, e2+0);
    __m128i vpacket1 = _mm_set_epi64x(e2+6, e2+4);
  ...
  if (uxymap[uxy]) {\
    u32 u = extract32(w,x);\
    for (u32 j = 0; j < PROOFSIZE; j++) {\
      if (cycleus[j] == u && cyclevs[j] == sipnode(&trimctx->sip_keys, edge+i, 1)) {\
